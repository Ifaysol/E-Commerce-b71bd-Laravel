...
<div class="panel">
    ...
    <div class="panel-heading bord-btm clearfix pad-all h-100">
        <h3 class="panel-title pull-left pad-no">{{ translate('Deleted Products') }}</h3>
        <div class="pull-right clearfix">
            <form class="" id="sort_products" action="" method="GET">
                @if($type == 'Seller')
                <div class="box-inline pad-rgt pull-left">
                <select name="published" class="form-control" onchange="sort_products()">
                            <option value="">Select Status</option>
                            <option @if(isset($_GET['published'])) @if($_GET['published'] == 0) selected  @endif @endif value="0">Unpublished</option>
                            <option @if(isset($_GET['published'])) @if($_GET['published'] == 1) selected  @endif @endif value="1">Published</option>
                        </select>
                        </div>
                    <div class="box-inline pad-rgt pull-left">
        ...
        <table class="table table-striped res-table mar-no" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <th class="d-none">{{translate('Todays Deal')}}</th>
                    <th class="d-none">{{translate('Rating')}}</th>
                    @if($type == 'Seller')
                        <th class="d-none">{{translate('Approved')}}</th>
                    @endif
                    <th class="d-none">{{translate('Published')}}</th>
                    <th class="d-none">{{translate('Featured')}}</th>
            ...
            <tbody>
                @foreach($products as $key => $product)
                    <tr>
                        ...
                        <td class="d-none"><label class="switch">
                                <input onchange="update_todays_deal(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->todays_deal == 1) echo "checked";?> >
                                <span class="slider round"></span></label></td>
                        <td class="d-none">{{ $product->rating }}</td>
                        <td class="d-none"><label class="switch">
                                <input onchange="update_approved(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->admin_approve == 1) echo "checked";?> >
                                <span class="slider round"></span></label></td>
                        <td class="d-none"><label class="switch">
                        ...
                        <td>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    @if ($type == 'Seller')
                                        <li class="d-none"><a href="{{route('products.seller.edit', encrypt($product->id))}}">{{translate('Edit')}}</a></li>
                                    @else
                                        <li class="d-none"><a href="{{route('products.admin.edit', encrypt($product->id))}}">{{translate('Edit')}}</a></li>
                                    @endif
                                    <li><a onclick="confirm_modal('{{route('products.permanently_destroy', $product->id)}}');">{{translate('Permanently Delete')}}</a></li>
                                    <li class="d-none"><a href="{{route('products.duplicate', $product->id)}}">{{translate('Duplicate')}}</a></li>
                                </ul>
                            </div>
                        </td>
                    </tr>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_approved(el){
            ...
            $.post('{{ route('products.update_approved') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Products approval status updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
