...
@section('content')
                    ...
                    <div class="">
                        <div class="row">
                            <div class="col-md-3 col-6">
                                <div class="dashboard-widget text-center green-widget mt-4 c-pointer">
                                    <a href="{{route('seller.products')}}" class="d-block">
                                        ...
                                        <span class="d-block title heading-3 strong-400">{{ count(\App\Product::where('user_id', Auth::user()->id)->where('published', '!=',2)->get()) }}</span>
                            ...
                            <div class="col-md-12">
                                <br>
                                @php
                                $flash_deals = App\FlashDeal::orderBy('created_at', 'desc')->where('status', 1)->get();
                                @endphp
                                @if(count($flash_deals) > 0)
                                <div class=" mt-4">
                                    <h5>Upcomming and Runnig Flash Deal and Campaign</h5>
                                    @foreach($flash_deals as $flash_deal)
                                    <div class="">
                                        <table class="table table-bordered table-sm shadow-sm bg-white">
                                            <thead>
      <tr>
        <th>Name</th>
        <th>Date From</th>
    ...
    <tbody>
      <tr>
        <td><strong>{{$flash_deal->title}}</strong></td>
        <td><strong>{{date('Y-m-d',$flash_deal->start_date)}}</strong></td>
        <td><strong> {{date('Y-m-d',$flash_deal->end_date)}}</strong></td>
        <td> <a href="{{route('flash_deals.edit_seller', encrypt($flash_deal->id))}}" class="alert-link">Add Prodcut</a></td>
      </tr>
