    	...
    	<div class="panel-body">
            ...
            <div class="row">
                ...
                <div class="col-lg-offset-6 col-lg-3">
                    ...
                    <select class="form-control demo-select2"  data-minimum-results-for-search="Infinity" id="update_payment_status">
                        ...
                        <option value="cancel" @if ($payment_status == 'cancel') selected @endif>{{translate('Cancel Order')}}</option>
                        <option value="return" @if ($payment_status == 'return') selected @endif>{{translate('Product Returned')}}</option>
                        <option value="curier_unpaid" @if ($payment_status == 'curier_unpaid') selected @endif>{{translate('Couier Unpaid')}}</option>
                    </select>
                </div>
                <div class="col-lg-3">
                    ...
                    <select class="form-control demo-select2"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                        ...
                        <option value="canceled" @if ($delivery_status == 'canceled') selected @endif>{{translate('Canceled')}}</option>
                        <option value="return" @if ($delivery_status == 'return') selected @endif>{{translate('Returned')}}</option>
                        <option value="rejected" @if ($delivery_status == 'rejected') selected @endif>{{translate('Rejected By Customer')}}</option>
                        <option value="missing" @if ($delivery_status == 'missing') selected @endif>{{translate('Missing Product')}}</option>
                    </select>
                </div>
            </div>
            <hr>
    		...
    		<div class="clearfix">
                ...
                <tr>
    				...
    				<td>
    					{{ single_price($order->orderDetails->where('seller_id', '!=',$admin_user_id)->max('shipping_cost')) }}
    				</td>
    			</tr>
    			<tr>
    				<td>
    				...
    				<td class="text-bold h4">
    					{{ single_price($order->orderDetails->where('seller_id', '!=',$admin_user_id)->sum('price') + $order->orderDetails->where('seller_id', '!=',$admin_user_id)->sum('tax') + $order->orderDetails->where('seller_id', '!=',$admin_user_id)->max('shipping_cost')) }}
    				</td>
    			</tr>
    			</tbody>
    			</table>
    		...
    		<div class="text-right no-print">
    			<a href="{{ route('seller.invoice.download_1', $order->id) }}" class="btn btn-default"><i class="demo-pli-printer icon-lg"></i></a>
    		</div>
    	</div>
    </div>
@endsection
...
@section('script')
    <script type="text/javascript">
        $('#update_delivery_status').on('change', function(){
            ...
            $.post('{{ route('orders.update_delivery_status_1') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,status:status}, function(data){
        ...
        $('#update_payment_status').on('change', function(){
            ...
            $.post('{{ route('orders.update_payment_status_1') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,status:status}, function(data){
