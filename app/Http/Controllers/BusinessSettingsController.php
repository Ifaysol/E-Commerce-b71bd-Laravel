...
class BusinessSettingsController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function env_key_update_sms(Request $request)
    {
        // dd($request->all());
        ...
        if ($business_settings) {
            $business_settings->value = $request->alpha_username;
            $business_settings->save();
        }
        else{
            $new_business_settings = new BusinessSetting;
            $new_business_settings->type = 'alpha_username';
            $new_business_settings->value = $request->alpha_username;
            $new_business_settings->save();
        }
        
        if ($business_settings2) {
            $business_settings2->value = $request->alpha_username;
            $business_settings2->save();
        }
        else{
            $new_business_settings2 = new BusinessSetting;
            $new_business_settings2->type = 'alpha_token';
            $new_business_settings2->value = $request->alpha_token;
            $new_business_settings2->save();
        }
    ...
    public function shipping_configuration_custom(Request $request){
        $business_settings = BusinessSetting::where('type', 'inside_dhaka')->first();
        $business_settings->value = $request->inside_dhaka;
        $business_settings->save();
        
        $business_settings1 = BusinessSetting::where('type', 'outside_dhaka')->first();
        $business_settings1->value = $request->outside_dhaka;
