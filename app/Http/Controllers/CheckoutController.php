...
class CheckoutController extends Controller
{
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        $subtotal = 0;
        $tax = 0;
        // $shipping = \App\BusinessSetting::where('type', $data['shipping'])->first()->value ?? 0;
        $shipping = 0;
        // dd($shipping);
        $request->session()->put('chargess', $shipping);
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if(Session::has('cart') && count(Session::get('cart')) > 0){
            ...
            // dd($cart);
            // dd(Session::get('delivery_info'));
            $request->session()->put('cart', $cart);

            $subtotal = 0;
            $tax = 0;
            // $request->session()->get('chargess');
            // dd(Session::get('chargess'));
            // $shipping = Session::get('chargess') ?? 0;
            $shipping = 0;
            foreach (Session::get('cart') as $key => $cartItem){
                $subtotal += $cartItem['price']*$cartItem['quantity'];
                $tax += $cartItem['tax']*$cartItem['quantity'];
                $pro = \App\Product::find($cartItem['id']);
                if(Session::get('shipping_info')['shipping'] == 'inside_dhaka') 
                {
                    $ship_amount = $pro->shipping_cost;
                } else {
                    $ship_amount = $pro->outside_shipping_cost;
                }
