...
class HomeController extends Controller
{
    ...
    {
        // return view('frontend.pre_sale');
        $products = filter_products(Product::whereNotNull('pre_sale')->orderBy(\DB::raw('-`serial`'), 'desc'))->get();
        return view('frontend.pre_sale', compact('products'));
        // return view('frontend.product_listing', compact('products'));
    }
    
    public function flash_deal_list(Request $request)
    {
        // return view('frontend.pre_sale');
        $products = FlashDeal::where('status', 1)->latest()->get();
        return view('frontend.flash_deal_list', compact('products'));
    }
    ...
    public function user_registration(Request $request)
    {
        ...
        if($request['mobile'] == null AND $request['email'] == null){
        ...
        $otp_code = rand(10001,99999);
            ...
            flash('Thank you for registration, A varification number has been send to your mobile')->success();
            $phone = $request['mobile'];
            // otp
    ...
    public function cart_login(Request $request)
    {
        ...
        if ($user->user_type == 'customer') {
            if ($user->active != 1) {
                ...
                flash(translate('Unverified user.'))->error();
                return back();
            }
        }
        if($user != null){
    ...
    {
        // $products = filter_products(Product::orderBy('created_at', 'desc'))->paginate(36);
        // return view('frontend.product_listing', compact('products'));
    ...
    {
        $keywords = array();
        $products = Product::where('published', 1)->where('tags', 'like', '%'.$request->search.'%')->orWhere('qr_code', 'like', '%'.$request->search.'%')->get();
        ...
        $products = filter_products(Product::where('published', 1)->where('name', 'like', '%'.$request->search.'%')->orWhere('qr_code', 'like', '%'.$request->search.'%'))->get()->take(3);
    ...
    public function search(Request $request)
    {
        ...
        $products = Product::where($conditions)->whereNull('pre_sale')->orderBy(\DB::raw('-`serial`'), 'desc');
        ...
        if($query != null){
            ...
            $products = $products->where('name', 'like', '%'.$query.'%')->orWhere('tags', 'like', '%'.$query.'%')->orWhere('qr_code', 'like', '%'.$query.'%');
        }
        ...
        $products = filter_products($products)->paginate(36)->appends(request()->query());
    ...
    public function campaign_link_store(Request $request)
    {
        BusinessSetting::where('type', 'campaign_link')->update(['value' => $request->campaign]);
        

        flash(translate('Campaign Link added successfully'))->success();
        return redirect()->route('home_settings.index');
    }
